...
function downvoteComments (url, request){
  ...
  const urlCommentID  = url.split("/").filter(segment => segment)[1]
  const requestUserName = request.body && request.body.username

  if ( request.body && urlCommentID && database.comments[urlCommentID] && requestUserName && database.users[requestUserName] ){
    if ( database.comments[urlCommentID].downvotedBy.indexOf(requestUserName) < 0){
      database.comments[urlCommentID].downvotedBy.push(requestUserName)
    }
    let upvotedArrayIndex =  database.comments[urlCommentID].upvotedBy.indexOf(requestUserName)
    if ( upvotedArrayIndex >= 0){
      database.comments[urlCommentID].upvotedBy.splice(upvotedArrayIndex,1)
    }
    response.body = {}
    response.body.comment = database.comments[urlCommentID]
    response.status = 200;    
  }else{
    response.status = 400;
  }
...
function upvoteComments(url,request){
  ...
  if ( request.body && urlCommentID && database.comments[urlCommentID] && requestUserName && database.users[requestUserName]){
    if ( database.comments[urlCommentID].upvotedBy.indexOf(requestUserName) < 0){
      database.comments[urlCommentID].upvotedBy.push(requestUserName)
    }

    let downvotedArrayIndex =  database.comments[urlCommentID].downvotedBy.indexOf(requestUserName)
    if ( downvotedArrayIndex >= 0){
      database.comments[urlCommentID].downvotedBy.splice(downvotedArrayIndex,1)
    }
    response.body = {}
    response.body.comment = database.comments[urlCommentID]
    response.status = 200;
...
function deleteComment(url, request ){
  ...
  const commentIDFromRequestUrl = url.split('/').filter(segment => segment)[1];
  if( commentIDFromRequestUrl && database.comments[commentIDFromRequestUrl] ){
    let author = database.comments[commentIDFromRequestUrl].username
    let art = database.comments[commentIDFromRequestUrl].articleId

    database.comments[commentIDFromRequestUrl] = null

    let userCommentsArrayIndex = database.users[author].commentIds.indexOf(Number(commentIDFromRequestUrl))
    database.users[author].commentIds.splice(userCommentsArrayIndex,1)

    let articleCommentsArrayIndex = database.articles[art].commentIds.indexOf(Number(commentIDFromRequestUrl))
    database.articles[art].commentIds.splice(articleCommentsArrayIndex,1)
...
if ( database.comments[commentIDFromRequestUrl]){
  let newComment = request && request.body && request.body.comment && request.body.comment.body;
  if (newComment){
    database.comments[commentIDFromRequestUrl].body = request && request.body && request.body.comment && request.body.comment.body;
    response.body={};
    response.body.comment = database.comments[commentIDFromRequestUrl];    
    response.status = 200;
  }else{
    if ( request && request.body && request.comment){
      response.status = 200;
    }else{
      response.status = 400;
    };
...
function createNewComment (url,request){
  const response = {};
  const bodyFromRequest = request.body &&  request.body.comment && request.body.comment.body
  const usernameFromRequest = request.body && request.body.comment && request.body.comment.username
  const articleIdFromRequest = request.body &&  request.body.comment && request.body.comment.articleId

if( database.users[usernameFromRequest] && bodyFromRequest && database.articles[articleIdFromRequest]){
  const comment = {
    id: database.nextCommentId++,
    body : request.body.comment.body,
    username : usernameFromRequest,
    articleId : articleIdFromRequest,
    commentIds: [],
    upvotedBy: [],
    downvotedBy: []
  }
  database.comments[comment.id]=comment;
  database.users[usernameFromRequest].commentIds.push(comment.id)
  database.articles[articleIdFromRequest].commentIds.push(comment.id)
  response.body={}
